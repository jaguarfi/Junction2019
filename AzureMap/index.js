...
var map, 
object1= {layer:{},data_source:{},n_people:0},
...
datasource, datasourceBeacon, client, popup, searchInput, resultsPanel, searchInputLength, centerMapOnResults,   time, shapes ,url;
        ...
        function slide(val) {
            ...
            // if(datasourceHeat_1){
                ...
                /////================/////
                time = (object1.data_source.shapes[val].getProperties().time_interval);
        ...
        function addLayer(dataObject){
            ...
            dataObject.layer = new atlas.layer.HeatMapLayer(dataObject.data_source, null, {
                weight: ['get', 'point_count'],
                radius: 15,
                opacity: 0.8
            });
        ...
        function changeRadius(dataObject,val){
            var pt = dataObject.layer.getOptions();
                console.log(pt.radius)
                 console.log(dataObject.n_people)
                 pt.radius = dataObject.n_people;

                 dataObject.layer.setOptions(pt);
                
                // shapes = datasourceHeat_1.getShapes();
                dataObject.n_people = parseInt(dataObject.data_source.shapes[val].getProperties().n_people);
                var alertMsg = parseFloat(dataObject.data_source.shapes[val].getProperties().hour_rank);

                  if(alertMsg > 0.7){
                    var  Http = new XMLHttpRequest();
                    var msg = "Abraham nearby the Beacon ";
                    msg = msg.concat(dataObject.data_source.shapes[val].getProperties().beacon_name);
                    msg = msg.concat(" the concentration of people is higher than ",alertMsg)
                    url ="https://api.telegram.org/bot1030965882:AAEH9qqrIMB5T2Ja5J7FO6sjL-z93_xaC6Y/sendMessage?chat_id=403339413&text="
                    url= url.concat(msg);
                     Http.open("GET", url);
                     Http.send();
